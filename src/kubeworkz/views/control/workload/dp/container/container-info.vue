  ...
  <div>
    ...
    >
      <dynamicTab
        ...
        validate-file="containers"
        :initial-add="true"
        :min-count="1"
        :mini-formatter="(item, index) => {
          return `Configuration-${index + 1}`
        }"
        :get-default-item="getDefaultContainer"
      >
          ...
          <div v-show="item.showAdvanced">
            ...
            >
              <template slot="label">
                ...
                <el-tooltip
                  effect="dark"
                  content="The init container does not support readiness probes and must be able to terminate. A pod can have multiple init containers, which will be executed sequentially before the business container is run."
                  placement="right"
              ...
              <el-radio-group v-model="item.type">
                <el-radio-button
                  v-for="typeItem in containerTypes"
                  :key="typeItem.value"
            ...
            <probe-config
              ...
              :prefix-key="`containers.${index}.probe.liveness`"
              probe="LivenessProbe"
            />
            <probe-config
              v-if="item.type === 'normal'"
              ...
              :prefix-key="`containers.${index}.probe.readiness`"
              probe="ReadyProbe"
            />
            ...
            <probe-config
              ...
              :prefix-key="`containers.${index}.probe.preStop`"
            ...
            <probe-config
              ...
              :prefix-key="`containers.${index}.probe.postStart`"
              probe="LifePostStopProbe"
            />
            <port-config
              key="ports"
              v-model="item.ports"
              :prefix-key="`containers.${index}.ports`"
            />
            <el-form-item
            ...
            >
              ...
              <el-radio-group v-model="item.imagePullPolicy">
                <el-radio-button
                  v-for="policy in imagePullPolicyList"
                  :key="policy.value"
                  :label="policy.value"
                >
                  {{ policy.text }}
